...
int Human::getArrowPressed() const {
...
void Human::setLifeStatus(bool x) {
  this->killed = !x;
}

void Human::getMoveDirection() {
  while (true) {
    std::cout << "Give directions for your character: ";
    char ch;
    ch = (char)getch();
    if(ch == '\000') {
      ch = (char) getch();
      switch (ch) {
        case KEY_UP:
          this->arrowPressed = KEY_UP;
          break;
        case KEY_DOWN:
          this->arrowPressed = KEY_DOWN;
          break;
        case KEY_LEFT:
          this->arrowPressed = KEY_LEFT;
          break;
        case KEY_RIGHT:
          this->arrowPressed = KEY_RIGHT;
          break;
        default:
          break;
      }
    }
    //std::cin >> this->arrowPressed;
