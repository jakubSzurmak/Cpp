...
void Organism::displayComment(char attacker, char defender, char statusFlag) const {
  switch (statusFlag) {
    ...
    case ESCAPED:
      ...
      std::cout << "has escaped attack of ";
      this->displayOrganismName(attacker);
      std::cout << std::endl;
      break;
    case RECREATION:
...
bool Organism::considerAction() {
  return true;
}

bool Organism::abilityActive(){
  return false;
}

int Organism::getCooldownReps(){
  return 0;
}

int Organism::getStrengthDecreaseReps(){
  return 0;
}
...
int Organism::findFreeRecreationSpot(Organism *partner) const {
  ...
  if ((this->boardField)->getYCord() == 1) {
    if ((this->boardField)->getXCord() == 1) {
      if (this->boardBeginning[this->position + M] == nullptr) {
        ...
        if (counter >= 2) {
          ...
          return partner->findFreeRecreationSpot(this->boardBeginning[partner->position]);
        }
      }
    ...
    if ((this->boardField)->getXCord() == 1) {
      ...
      } else if (this->boardBeginning[this->position + M] == nullptr) {
        return this->position + M;
      ...
      if (this->boardBeginning[this->position - (this->movementSpeed * M)] == nullptr) {
        return this->position - (this->movementSpeed * M);
        ...
        return this->position + this->movementSpeed;
